"""
make_graphs_atomic_minimal_feats_with_edge_attr.py
...
Usage:
  python prepare_atlas.py <PDB_CHAIN> [--selection <SEL>] [--property <PROP>] [--output <OUT>] [--datasets_dir <DIR>]

Example:
  python prepare_atlas.py 1MBN_A
  python prepare_atlas.py 1MBN_A --selection "protein and backbone" --property rog
"""
...
# File discovery
# ----------------------------
def find_trajectory_files(pdb_chain: str, datasets_dir: str = "datasets") -> tuple:
    """
    Find .xtc and .pdb files in datasets/{PDBID}_{Chain}_protein/ folder.
    
    Args:
        pdb_chain: PDB ID and chain (e.g., "1MBN_A")
        datasets_dir: Base datasets directory
        
    Returns:
        (xtc_path, pdb_path) as absolute paths
    ...
    # Find .xtc and .pdb files
    xtc_files = list(folder_path.glob("*.xtc"))
    pdb_files = list(folder_path.glob("*.pdb"))
    
    if not xtc_files:
        raise FileNotFoundError(f"No .xtc file found in {folder_path}")
    if not pdb_files:
        raise FileNotFoundError(f"No .pdb file found in {folder_path}")
    
    # Use the first match (or could filter by naming convention)
...
def main():
    parser = argparse.ArgumentParser(
        description="Prepare molecular dynamics trajectories from ATLAS as PyTorch Geometric graphs",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python prepare_atlas.py 1MBN_A
  python prepare_atlas.py 1MBN_A --selection "protein and backbone" --property rog
  python prepare_atlas.py 1MBN_A --output graphs.pkl --datasets_dir /path/to/datasets
        """
    )
    
    parser.add_argument('pdb_chain', help='PDB ID and chain (e.g., 1MBN_A)')
    ...
    if args.output is None:
        args.output = f"{args.pdb_chain}_graphs.pkl"
    
    print(f"[info] Processing {args.pdb_chain}")
    print(f"[info] Selection: '{args.selection}'")
    print(f"[info] Property: {args.property}")
    
    # Find files
    try:
        xtc_path, pdb_path = find_trajectory_files(args.pdb_chain, args.datasets_dir)
